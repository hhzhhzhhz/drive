...
func (tun *NativeTun) Write(buf []byte) (int, error) {
	...
	if cap(tun.wBuf) < len(buf)+4 {
		tun.wBuf = make([]byte, len(buf)+4)
	}
	tun.wBuf = tun.wBuf[:len(buf)+4]
	tun.wBuf[0] = 0x00
	tun.wBuf[1] = 0x00
	tun.wBuf[2] = 0x00
	switch buf[0] >> 4 {
	case 4:
		tun.wBuf[3] = unix.AF_INET
	case 6:
		tun.wBuf[3] = unix.AF_INET6
	default:
		return 0, unix.EAFNOSUPPORT
	}
	copy(tun.wBuf[4:], buf)
	n, err := tun.tunFile.Write(tun.wBuf)
	return n - 4, err
}
